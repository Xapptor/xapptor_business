...
get_resumes_labels({
  ...
  if (main_resume_exists) {
    ...
    if (main_resume.creation_date == Resume.empty().creation_date) {
      ...
      String main_date_String =
          date_format.format(main_resume.creation_date.toDate());
  ...
  for (int i = 1; i <= loop_limit; i++) {
    ...
    if (resume_exists) {
      var current_resume =
          resumes.firstWhere((resume) => resume.slot_index == i);
      String backup_date_String =
          date_format.format(current_resume.creation_date.toDate());

      label =
          "$backup_label ${current_resume.slot_index} - $backup_date_String";
    } else {
